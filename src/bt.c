...
// Project
#include "io.h"
#include "lumonade.h" // Generated from lumonade.gatt

// TODO(eustas): where does it come from?
extern uint8_t const profile_data[];

// TODO(eustas): this is a toy characteristic.
static uint16_t magic_smoke = 42;
...
// Shortcut.
#define HANDLE(S)                                                              \
  ATT_CHARACTERISTIC_ORG_BLUETOOTH_CHARACTERISTIC_TEMPERATURE_01_##S##_HANDLE
...
uint16_t att_read_callback(hci_con_handle_t connection_handle,
  ...
  if (att_handle == HANDLE(VALUE)) {
    return att_read_callback_handle_blob((const uint8_t *)&magic_smoke,
                                         sizeof(magic_smoke), offset, buffer,
                                         buffer_size);
  }
  return 0;
}
...
int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle,
  ...
  if (att_handle != HANDLE(CLIENT_CONFIGURATION))
    return 0;
